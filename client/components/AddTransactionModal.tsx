            ...
            <div className={`flex gap-2 flex-1 min-h-0 ${isRightMode ? "flex-row-reverse" : ""}`}>
              ...
              <div className="grid grid-cols-3 gap-1.5 min-h-0 h-full" style={{ width: `${numpadSize}%`, gridTemplateRows: 'repeat(4, 1fr)', flex: 'none' }}>
                {(isRightMode
                  ...
                  if (btn === 'save') return (
                    <button key="save" onClick={handleAmountSave} className="h-full px-2 bg-gradient-to-br from-theme-500 to-theme-600 hover:from-theme-600 hover:to-theme-700 text-theme-on font-bold rounded-xl transition-all active:scale-95 shadow-md">Save</button>
                  );
                  if (btn === '.') return (
