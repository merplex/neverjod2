...
export default function TransferModal({ editRepeatId, onClose, onSaved }: TransferModalProps) {
  ...
  const canSave = fromId && toId && fromId !== toId && parseFloat(amount) > 0;
          ...
          <div className="overflow-y-auto px-5 pb-4 space-y-4 flex-1">
            ...
            <div>
              ...
              >
                {toAcc ? (
                  <>
                    <span className="font-semibold text-slate-800">{toAcc.name}</span>
                    <span className={`text-xs ${toAcc.currentBalance >= 0 ? "text-slate-500" : "text-red-500"}`}>
            ...
            <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
              ...
              {repeatEnabled && (
                ...
                >
                  <span className="text-xs text-slate-400 w-20 text-left">Frequency</span>
          ...
          <div className="relative bg-white rounded-2xl w-full max-w-sm max-h-[70vh] flex flex-col shadow-xl">
            ...
            <div className="overflow-y-auto">
              {accounts.map((acc) => (
                <button
                  key={acc.id}
                  onClick={() => { setToId(acc.id); setShowToPicker(false); }}
                  className={`w-full flex items-center justify-between px-5 py-3.5 hover:bg-slate-50 transition-colors border-b border-slate-100 last:border-b-0 ${acc.id === toId ? "bg-theme-50" : ""}`}
                >
                  <span className={`text-sm font-semibold ${acc.id === toId ? "text-theme-700" : "text-slate-800"}`}>{acc.name}</span>
                  <div className="flex items-center gap-2">
                    <span className={`text-xs ${acc.currentBalance >= 0 ? "text-slate-400" : "text-red-500"}`}>{cur}{acc.currentBalance.toLocaleString()}</span>
                    {acc.id === toId && <span className="text-theme-600 text-base">✓</span>}
                  </div>
                </button>
              ))}
            </div>
